...
4. Li /et al/ (2017) use a fixed mean latency period of 4 days and an
   ...
   - There is a wider range for the duration of infeciousness in the
...
5. A very rough estimate of the detection probability is 1 in 200. If
   ...
   - We assume that the observation process changed once 153 days pre-present
...
8. Because it seems likely that there was effort to sequence the first
   ...
   place.
9. *TODO* Ensure the following assumptions are met:
   - [ ] Any changes to parameter values are assumed to occur at midnight.
   - [ ] To avoid any overlap with parameter changes times or sample times, these estimates are done at 9AM on each day of interest (date + 0.375).
